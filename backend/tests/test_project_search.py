...
async def test_search_project_returns_results(
    ...
    create_response = await client.post("/api/v1/projects", json={"name": "Search Test"})
...
@pytest.mark.asyncio
async def test_search_project_not_found(client: AsyncClient) -> None:
...
@pytest.mark.asyncio
async def test_search_semantic_search_unavailable(client: AsyncClient) -> None:
    create_response = await client.post("/api/v1/projects", json={"name": "SQLite Search"})
    project_id = create_response.json()["id"]
...
@pytest.mark.asyncio
async def test_search_rejects_invalid_body(client: AsyncClient) -> None:
    create_response = await client.post("/api/v1/projects", json={"name": "Invalid Search"})
    project_id = create_response.json()["id"]
